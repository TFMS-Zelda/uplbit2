...
<section class="content">
    ...
    <section class="bg-primary text-center">
        <div class="container">
            <h2> <i class="fas fa-wrench"></i> CONTROL Y REGISTRO MANTENIMIENTO TI </h2>
        </div>
        <!-- /.container-->
    </section>
    ...
    <section>
        <div class="container">
            <div class="alert alert-success" role="alert">
                ...
                <p class="mb-0">
                    {{ Auth::user()->name }}, a continuación registrara un mantenimiento al equipo de computo:
                    <em>{{ $computer->brand }} {{ $computer->model }}</em> <br>
                    ServiceTag Serial: <strong>{{ $computer->servicetag }}</strong> <br>
                    Placa corporativa: <strong>{{ $computer->license_plate }}</strong>
                    <hr>
                    Log... Última fecha de actualización: {{ $computer->updated_at->diffForHumans() }},
                    {{ Carbon\Carbon::parse($computer->updated_at)->format('l jS \\of F Y ') }}
                </p>
            </div>
        </div>
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <select class="form-control" name="computer_id" required autofocus>
                            <option value="{{ $computer->id }} ">{{ $computer->brand}}
                                - {{ $computer->model }}, {{ $computer->servicetag }}
                            </option>
                        </select>
                        <small class="form-text text-gray-600">
                            El Equipo de computo es detectado automáticamente por el sistema, la placa corporativa de el
                            equipo de computo
                            es <strong><code>{{ $computer->license_plate }} </code></strong>...
                        </small>
                    </div>
                </div>
            </div>
        </div>
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalCreateMaintanceComputer">
            Enviar
        </button>

        <!-- Modal -->
        <div aria-hidden="true" aria-labelledby="modalCreateMaintanceComputer" class="modal fade"
            id="modalCreateMaintanceComputer" role="dialog" tabindex="-1">
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">
                                                ¿Registrar Mantenimiento de equipo de computo?
                                            </div>
                                            <div class="p mb-0 font-weight text-gray-800">
                                                <p>
                                                    ...
                                                    {{ Auth::user()->name }}
                                                    <br>
                                                    ¿Desea registrar el siguiente mantenimiento de equipo de computo?
                                                    <div class="alert alert-success" role="alert">
                                                        <h4 class="alert-heading">
                                                            <i class="fa fa-bell" aria-hidden="true"></i>
                                                            Nota!
