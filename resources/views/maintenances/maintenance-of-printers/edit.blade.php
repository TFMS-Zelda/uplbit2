...
<section class="content">
    ...
    <section class="bg-primary text-center">
        <div class="container">
            <h2> <i class="fas fa-wrench"></i> EDITAR MANTENIMIENTO IMPRESORA N° {{ $historyMaintenance->id }}
            </h2>
        </div>
        <!-- /.container-->
    </section>
    ...
    <section>
        <div class="container">
            <div class="alert alert-warning" role="alert">
                ...
                <p class="mb-0">
                    {{ Auth::user()->name }}, a continuación editara un mantenimiento registrado de la impresora:
                    <em>{{ $historyMaintenance->printer->brand }} {{ $historyMaintenance->printer->model }}</em> <br>
                    Serial: <strong>{{ $historyMaintenance->printer->serial }}</strong> <br>
                    Placa corporativa: <strong>{{ $historyMaintenance->printer->license_plate }}</strong>
                    <hr>
                    Log... Última fecha de actualización:
                    {{ $historyMaintenance->printer->updated_at->diffForHumans() }},
                    {{ Carbon\Carbon::parse($historyMaintenance->printer->updated_at)->format('l jS \\of F Y ') }}
                </p>
            </div>
        </div>
    ...
    <form action="{{ route('maintenances.maintenance-of-printers.update', $historyMaintenance->id) }}" method="POST"
        ...
        <p class="h4 mb-1 text-gray-800"><i class="fas fa-laptop"></i>Impresora</p>

        <div class="row">
            <div class="col-md-12">
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <select class="form-control" name="printer_id" required autofocus>
                            <option value="{{ $historyMaintenance->printer->id }} ">
                                {{ $historyMaintenance->printer->brand}}
                                - {{ $historyMaintenance->printer->model }},
                                {{ $historyMaintenance->printer->servicetag }}
                            </option>
                        </select>
                        <small class="form-text text-gray-600">
                            La impresora es detectada automáticamente por el sistema, la placa corporativa de la
                            impresora
                            es <strong><code>{{ $historyMaintenance->printer->license_plate }} </code></strong>...
                        </small>
                    </div>
                </div>
            </div>
        </div>
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalUpdatePrinter">
            Actualizar
        </button>


        <!-- Modal -->
        <div aria-hidden="true" aria-labelledby="modalUpdatePrinter" class="modal fade" id="modalUpdatePrinter"
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">
                                                ¿Actualizar Mantenimiento de impresora?
                                            </div>
                                            <div class="p mb-0 font-weight text-gray-800">
                                                <p>
                                                    ...
                                                    {{ Auth::user()->name }}
                                                    <br>
                                                    ¿Desea actualizar el siguiente mantenimiento de impresora?
                                                    <div class="alert alert-warning" role="alert">
                                                        <h4 class="alert-heading">
                                                            <i class="fa fa-bell" aria-hidden="true"></i>
                                                            Nota!
                                                        ...
                                                        <p class="mb-0">
                                                            ...
                                                            <em>{{ $historyMaintenance->printer->brand }}
                                                                {{ $historyMaintenance->printer->model }}</em> <br>
                                                            Serial:
                                                            <strong>{{ $historyMaintenance->printer->servicetag }}</strong>
                                                            <br>
                                                            Placa corporativa:
                                                            <strong>{{ $historyMaintenance->printer->license_plate }}</strong>
                                        ...
                                        <div class="col-auto">
                                            <i class="fas fa-printer fa-2x text-gray-300">
                                            </i>
                                        </div>
                                    </div>
                                </div>
