...
<section class="content">
    ...
    <section class="bg-primary text-center">
        <div class="container">
            <h2> <i class="fas fa-wrench"></i> CONTROL Y REGISTRO MANTENIMIENTO DE IMPRESORAS </h2>
        </div>
        <!-- /.container-->
    </section>
    ...
    <section>
        <div class="container">
            <div class="alert alert-success" role="alert">
                ...
                <p class="mb-0">
                    {{ Auth::user()->name }}, a continuación registrara un mantenimiento a la impresora:
                    <i class="fas fa-print"></i>
                    <em>{{ $printer->brand }} {{ $printer->model }}</em> <br>
                    Serial: <strong>{{ $printer->serial }}</strong> <br>
                    Placa corporativa: <strong>{{ $printer->license_plate }}</strong>
                    <hr>
                    Log... Última fecha de actualización: {{ $printer->updated_at->diffForHumans() }},
                    {{ Carbon\Carbon::parse($printer->updated_at)->format('l jS \\of F Y ') }}
                </p>
            </div>
        </div>
    ...
    <form action="{{ route('maintenances.maintenance-of-printers.store') }}" method="POST"
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <select class="form-control" name="printer_id" required autofocus>
                            <option value="{{ $printer->id }} ">{{ $printer->brand}}
                                - {{ $printer->model }}, {{ $printer->serial }}
                            </option>
                        </select>
                        <small class="form-text text-gray-600">
                            La impresora es detectado automáticamente por el sistema, la placa corporativa de la
                            impresora
                            es <strong><code>{{ $printer->license_plate }} </code></strong>...
                        </small>
                    </div>
                </div>
            </div>
        </div>
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalCreateMaintancePrinter">
            Enviar
        </button>

        <!-- Modal -->
        <div aria-hidden="true" aria-labelledby="modalCreateMaintancePrinter" class="modal fade"
            id="modalCreateMaintancePrinter" role="dialog" tabindex="-1">
                            ...
                            <div class="card border-left-primary shadow h-100 py-2">
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="p mb-0 font-weight text-gray-800">
                                                <p>
                                                    ...
                                                    {{ Auth::user()->name }}
                                                    <br>
                                                    ¿Desea registrar el siguiente mantenimiento de la impresora?
                                                    <div class="alert alert-success" role="alert">
                                                        <h4 class="alert-heading">
                                                            <i class="fa fa-bell" aria-hidden="true"></i>
                                                            Nota!
                                                        ...
                                                        <p class="mb-0">
                                                            ...
                                                            <em>{{ $printer->brand }} {{ $printer->model }}</em> <br>
                                                            Serial:
                                                            <strong>{{ $printer->serial }}</strong> <br>
                                                            Placa corporativa:
                                                            <strong>{{ $printer->license_plate }}</strong>
                                        ...
                                        <div class="col-auto">
                                            <i class="fas fa-print fa-2x text-gray-300">
                                            </i>
                                        </div>
                                    </div>
                                </div>
